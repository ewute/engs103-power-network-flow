...
## Data Sources
Our model is built on highly credible, real-world data to ensure the network accurately reflects the physical and economic realities of the modern power grid:
- **Transmission Line Capacity (400 MW per line):** The thermal limit for our transmission lines is strictly informed by the U.S. Department of Energy's *National Transmission Grid Study*. A standard 230 kV High-Voltage Alternating Current (HVAC) is physically capped around 400 MW to prevent the cables from sagging or melting.
- **Power Plant Costs & Generation Capacities:** The operational costs (marginal cost of electricity production per MWh) and maximum power outputs for each plant type (Nuclear, Hydroelectric, Natural Gas, and Coal) are derived from the U.S. Energy Information Administration's (EIA) *Levelized Cost of Energy (LCOE)* reports. This ensures our model accurately prefers cheap baseload power over expensive peaker plants.
- **Town & Power Plant Locations:** The demand centers represent 10 actual major cities and towns in New Hampshire. The power plants represent real facilities in the region (e.g., Seabrook Nuclear Power Plant, Merrimack Station). Latitude and longitude coordinates for all nodes were manually gathered via exact Google Maps searches to calculate precise geographic distances.
- **Plant-to-Town Distances:** Straight-line geographic distances between nodes were computed using the Haversine formula on the Google Maps coordinate data (stored in `distance_pairs.csv`), which serves as the basis for our $0.05/km/MW transmission friction penalty.
- **Town Demands:** Peak load requirements for each town are scaled estimates roughly proportional to their actual relative populations and industrial footprints.
...
 * * * 
 
 # #   >��  A I   D i s c l o s u r e 
 P a r t s   o f   t h i s   w r i t e - u p ,   a s   w e l l   a s   t h e   a c c o m p a n y i n g   r e p o s i t o r y   R E A D M E ,   w e r e   g e n e r a t e d   a n d   r e f i n e d   w i t h   d r a f t i n g   a s s i s t a n c e   f r o m   A I   t o o l s   ( s u c h   a s   G i t H u b   C o p i l o t ) .   H o w e v e r ,   t h e   c o r e   m a t h e m a t i c a l   f o r m u l a t i o n s ,   d a t a   c o l l e c t i o n ,   a n a l y t i c a l   c o n c l u s i o n s ,   a n d   c o d e   l o g i c   r e m a i n   m a n u a l l y   d i r e c t e d   o r i g i n a l   w o r k   a n d   h a v e   b e e n   r i g o r o u s l y   t e s t e d   a n d   v a l i d a t e d   b y   t h e   a u t h o r .  
 